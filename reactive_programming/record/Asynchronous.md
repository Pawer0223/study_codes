...
- 결국, 새로운 workthread가 요청한 갯수만큼 생성되어서 처리되고 있게된다.
  - 즉, 서블릿 요청에 대한 응답이 빠르다고 성능이 좋아진 것이아니라는 것이다.
  - 새로운 쓰레드에서 동작하는 워커 스레드에서 이루어지는 I/O와 같은 작업에서 Blocking이 될 가능성은 분명히 존재한다.

### Thread Hell
- 요청이 몰리는 상황. 위에서 100개의 요청을 빠르게 받을 수 있지만, 짧은 시간에 100개의 worker thread가 생성된다.
...
> - 생성되는 Worker Thread의 갯수를 적절히 제한한다.
>   - 이러한 역할을 Netty 서버가 해준다.
> - 여러개의 서비스가 호출되는 경우, Call Back으로 묶는다.
>   - 이 때, 서비스A, 서비스B가 연쇄적으로 호출이되는 상황이라면 모두 Netty서버에 의해 호출.
> - 서비스A가 2초, 서비스 B가3초가 소요되며, 동시에 100개의 요청이 들어온다면 ?
>   - 평균적으로 5초의 시간에 모든 요청을 처리할 수 있다.
>   - 여기서 5초라는 퍼포먼스는 2초 + 3초의 작업의 최소시간을 비동기적으로 거의 동시에 처리할 수 있기 때문에 가능한 것.
>   - 추가적으로, Thread Hell도 발생시키지 않을 수 있다.
